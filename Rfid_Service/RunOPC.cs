    ...
    public class RunOpc
    {
            ...
            if (!File.Exists(AppDomain.CurrentDomain.BaseDirectory + "Settings\\OpcConfiguration\\config.xml"))
            {
                if (config.Auth.AnonymousLogin != null && config.Auth.AnonymousLogin == false)
                    try
                    {
                        ...
                        _client.Security.UserIdentity =
                            new OpcClientIdentity(config.Auth.Username, config.Auth.Password);
                        _client.Security.EndpointPolicy = new OpcSecurityPolicy(OpcSecurityMode.SignAndEncrypt,
                            OpcSecurityAlgorithm.Auto);
                    ...
                    catch (Exception e)
                    {
                        _eventLog.WriteEntry(
                            $"Error with OPC Certification.\n{e.Message}\n\n{e.InnerException}\n\n{e.StackTrace}",
                            EventLogEntryType.Error, (int)EventIds.Ids.OpcCertificateError);
                        Environment.Exit(1);
                    }
                else
                    _client.Security.AutoAcceptUntrustedCertificates = true;
                ...
                _client.Configuration.SaveToFile(AppDomain.CurrentDomain.BaseDirectory +
                                                 "Settings\\OpcConfiguration\\config.xml");
            }
            else
            {
                _client.Configuration =
                    OpcApplicationConfiguration.LoadClientConfigFile(AppDomain.CurrentDomain.BaseDirectory +
                                                                     "Settings\\OpcConfiguration\\config.xml");
                if (config.Auth.AnonymousLogin != null && config.Auth.AnonymousLogin == false)
                    try
                    {
        ...
        public int SubscribeNode(OpcNodeId nodeId)
        {
            ...
            catch (OpcException e)
            {
                _eventLog.WriteEntry(
                    $"Connection Failed. \n\tError Code:{e.Code}\n\tMessage:{e.Message}\n\tCause:{e.Result}",
                    EventLogEntryType.Error, (int)EventIds.Ids.OpcConnectionFailure);
                return 1;
            }
        }
        ...
        public void TagValueChanged(object sender, OpcDataChangeReceivedEventArgs e)
        {
            ...
            if (nodeId.ToString().ToLower().Contains("write"))
                ...
                _eventLog.WriteEntry("No option on this node. Please revise", EventLogEntryType.Warning,
                    (int)EventIds.Ids.OpcTagNotRecognized);
        }
        ...
        private int WriteTagChange(OpcNodeId nodeId)
        {
            try
            {
                ...
                {
                    _eventLog.WriteEntry("Unable to find matching config. Check system settings.",
                        EventLogEntryType.Error, (int)EventIds.Ids.NullReaderError);
        ...
        private static void ClientOnCertificateValidationFailed(object sender,
